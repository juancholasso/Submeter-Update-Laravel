       ...
       columns.map(
           (c)=>{
               if(c.action != null){
                    ...
                    const fun = (dt)=>{
                        switch (c.action.event) {
                            case 'remove':
                                this.props.onRemove(dt)
                                break;
                            case 'update':
                                this.props.onUpdate(dt)
                                break;
                            case 'assign':
                                this.props.onAssign(dt)
                                break;
                            default:
                                break;
                        }
                    }
                    c.createdCell = (td, cellData, rowData) => {
                        return ReactDOM.render(
                            <button type="button" className={c.action.className}  onClick={()=>fun(rowData)}><span className={c.action.icon}></span></button>
                            ,td
                        );
                    }
                    //c.action = null
