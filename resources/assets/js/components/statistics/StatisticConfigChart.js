      ...
      {loading == false && !error && (
        <div className="plot-tab mx-0 px-0 production-plot">
          ...
          <div className="row mt-3 mb-3">
            <div className="col-6">
              <button
                className="btn btn-default float-left color-127"
                onClick={exportExcel}
              >
            ...
            <div className="col-6">
              <button
                onClick={exportPdf}
                className="btn btn-default float-right color-127 "
              >
                {!loadingPdf && (
                  <span>
          ...
          {details.rows && details.rows.length > 0 && (
            <div
              className="table-detail mt-5"
              style={{ width: "90%", margin: "0 auto" }}
            >
              <div className="justify-content-md-center">
                <table
                  id={`details_${props.configId}`}
                  className="table table-bordered table-sm"
                >
                  <thead style={{ background: "#F5F5F6" }}>
                    {details.header && (
                      <tr>
                        <th>{details.header.interval}</th>
                        {details.header.fields.length > 0 &&
                          details.header.fields.map((t, i) => (
                            <th
                              key={i}
