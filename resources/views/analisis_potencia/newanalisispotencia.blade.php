...
@section('counters')
	@include('Dashboard.includes.contadores', ["menuId" => 4])
@endsection
	...
	<div class="pdf-header">
		<div class="container" style="width:100%; display: inline-block">
			<div class="row">
				...
				<div class="col">
					<h5 style="text-align: center;">Informe Analisis de Potencia<h5>
				</div>
				<div class="col">
...
<div class="row">
	<div class="column">
		...
		<div class="btn-container">
			{{-- <button type="button" class="btn" id="button_optimizacion" onclick="mostrar_Optimizacion({{$id}})">OPTIMIZACIÓN POTENCIA CONTRATADA</button> --}}
			<button type="button" class="btn" id="button_optimizacion" onclick="mostrar_Optimizacion()">OPTIMIZACIÓN POTENCIA CONTRATADA</button>
			<button type="button" class="btn" id="exportButton"> GENERAR PDF</button>
		</div>
	</div>
</div>

<div id="optimizacion" class="d-none wrapper">
	@if($tipo_tarifa == 1)
		<div class="row row-2">
			<div class="column">
				<h3 class="column-title">Situación Actual</h3>
				<div class="table-container">
					<table class="table-responsive table-wide text-center">
						<thead>
							<tr class="row-header">
								<th></th>
			...
			<div class="column">
				<h3 class="column-title">Situación Óptima</h3>
				<div class="table-container">
					<table class="table-responsive table-wide text-center">
						<thead>
							<tr class="row-header">
								<th></th>
								@foreach($arreglo_potencia_optima as $potencia)
									<th>
										{{$potencia["periodo"]}}<br> (kW)
									</th>
								@endforeach
			...
			<div class="column">
				<div class="table-container">
					<table class="table-responsive table-wide text-center">
						...
						<tbody>
							@foreach($data_analisis["dataFP_optimal"] as $idx=>$data)
								<tr class="text-center" style="vertical-align: middle;">
									<td>P{{$idx}}</td>
									<td>{{number_format($data_analisis["dataFPC_optimal"][$idx],2,',','.')}} €</td>
									<td>{{number_format($data_analisis["dataFPE_optimal"][$idx],2,',','.')}} €</td>
									<td>{{number_format($data_analisis["dataFP_optimal"][$idx],2,',','.')}} €</td>
								</tr>
							@endforeach
						</tbody>
						<tfoot>
							<tr class="row-header">
								...
								<th>
									{{number_format($data_analisis["totalFC_optimal"],0,',','.')}} €
								</th>
								<th>
									{{number_format($data_analisis["totalFPE_optimal"],0,',','.')}} €
								</th>
								<th>
									{{number_format($data_analisis["totalFP_optimal"],0,',','.')}} €
								</th>
							</tr>
						</tfoot>
					</table>
					...
					<div class="table-container">
						<table class="table-responsive table-wide text-center">
							...
							<tbody>
								...
								<tr>
									<td>SITUACIÓN ÓPTIMA</td>
									<td>{{number_format($data_analisis["totalFC_optimal"],0,',','.')}} €</td>
									<td>{{number_format($data_analisis["totalFPE_optimal"],0,',','.')}} €</td>
									<td>{{number_format($data_analisis["totalFP_optimal"],0,',','.')}} €</td>
								</tr>
							</tbody>
						</table>
					</div>
					<div class="btn-container @if($diff_dates->days < 27) reverse @endif">
						@if($diff_dates->days >= 27)
							<form class="d-iblock" method="POST" action="{{route('analisis.potencia.optima')}}">
								{{ csrf_field() }}
								<input type="hidden" name="date_from" value="{{$date_from}}" />
								<input type="hidden" name="date_to" value="{{$date_to}}" />
								<input type="hidden" name="cl" value="{{$contador_label}}" />
								<button type="submit" class="btn">Calcular Potencia Óptima</button>
							</form>
						@endif
						<a class="btn" href="{{route('analisis_potencia_envio_email')}}">Solicitar Informe Ampliado</a>
					</div>
				</div>
			</div>
		</div>
				...
				<table class="table-analisis-comparacion tabla1 table table-bordered table-hover table-responsive">
					...
					<tbody>
						...
						<tr>
							...
							<td class="text-center" style="vertical-align: middle;">{{number_format($data_analisis["totalFC_optimal"],0,',','.')}} €</td>
							<td class="text-center" style="vertical-align: middle;">{{number_format($data_analisis["totalFPE_optimal"],0,',','.')}} €</td>
							<td class="text-center" style="vertical-align: middle;">{{number_format($data_analisis["totalFP_optimal"],0,',','.')}} €</td>
						</tr>
					</tbody>
				</table>
			</div>
				...
				<div class="column">
					<div class="table-container">
						<table class="table-responsive table-wide text-center">
							...
							<tbody>
								@foreach($data_month["dataFP_optimal"] as $idx=>$data)
									<tr>
										<td>P{{$idx}}</td>
										<td>{{number_format($data_month["dataFPC_optimal"][$idx],2,',','.')}} €</td>
										<td>{{number_format($data_month["dataFPE_optimal"][$idx],2,',','.')}} €</td>
										<td>{{number_format($data_month["dataFP_optimal"][$idx],2,',','.')}} €</td>
									</tr>
								@endforeach
							</tbody>
							<tfoot>
								<tr class="row-header">
									...
									<th>
										{{number_format($data_month["totalFPC_optimal"],0,',','.')}} €
									</th>
									<th>
										{{number_format($data_month["totalFPE_optimal"],0,',','.')}} €
									</th>
									<th>
										{{number_format($data_month["totalFP_optimal"],0,',','.')}} €
									</th>
								</tr>
							</tfoot>
						</table>
					...
					<div class="table-container">
						<table class="table-responsive table-wide text-center">
							...
							<tbody>
								...
								<tr>
									<td>SITUACIÓN ÓPTIMA</td>
									<td>{{number_format($data_month["totalFPC_optimal"],0,',','.')}} €</td>
									<td>{{number_format($data_month["totalFPE_optimal"],0,',','.')}} €</td>
									<td>{{number_format($data_month["totalFP_optimal"],0,',','.')}} €</td>
								</tr>
							</tbody>
						</table>
					</div>
				...
				<div class="col-md-6 export-pdf" data-pdforder="{{10 + 4*($index) + 3}}"><br><br>
					<table class="table-analisis-comparacion tabla1 table table-bordered table-hover table-responsive">
						...
						<tbody>
							...
							<tr>
								<td class="text-center" style="vertical-align: middle;">SITUACIÓN ÓPTIMA</td>
								<td class="text-center" style="vertical-align: middle;">{{number_format($data_month["totalFPC_optimal"],0,',','.')}} €</td>
								<td class="text-center" style="vertical-align: middle;">{{number_format($data_month["totalFPE_optimal"],0,',','.')}} €</td>
								<td class="text-center" style="vertical-align: middle;">{{number_format($data_month["totalFP_optimal"],0,',','.')}} €</td>
							</tr>
						</tbody>
					</table>
				</div>
	...
	@else
		...
		<div class="row">
			<div class="column">
				...
				<div class="btn-container">
					@if($diff_dates->days >= 27)
						<form class="ib-form" method="POST" action="{{route('analisis.potencia.optima')}}">
							{{ csrf_field() }}
							<input type="hidden" name="date_from" value="{{$date_from}}" />
							<input type="hidden" name="date_to" value="{{$date_to}}" />
							<input type="hidden" name="cl" value="{{$contador_label}}" />
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		createPlots();
		// $(".tab-plot-header").click(renderTabContent);
		$(".tab-link").click(renderTabContent);
