...
def read_data(conexion, dates):
	...
	try:
		...
		try:
			...
			for row in cursor:
				auxRow = [int(row[0]), int(row[1]), float(row[2]), int(row[3]), int(row[4])]
				dataPotencia.append(auxRow)

		precioTerminoPotencia = np.zeros(3)
...
def funct_consumo(x0):
	...
	maxMonth = 1200
	x0a = np.array(x0)
	powerCost = np.zeros(3)
	dataAnios = -1e152 * np.ones((3, maxMonth))
	limitDays = np.zeros((2, maxMonth))
	mesesDisponibles = 12.0
	diasPotencia = 365.0

	powerDayCost = precioTerminoPotencia * mesesDisponibles / diasPotencia
	...
	for i in range(0,len(dataPotencia)):
		...
		idxPotencia = dataPotencia[i][3] - 1
		if(dataPotencia[i][2] > dataAnios[idxPotencia][idxMonth]):
			dataAnios[idxPotencia][idxMonth] = dataPotencia[i][2]
		if(dataPotencia[i][4] < limitDays[0][idxMonth]):
			limitDays[0][idxMonth] = dataPotencia[i][4]
		if(dataPotencia[i][4] > limitDays[1][idxMonth]):
			limitDays[1][idxMonth] = dataPotencia[i][4]

	for idxMonth in range(0, maxMonth):
		if(limitDays[1, idxMonth] > 0):
			for i in range(0,3):
				pmax = dataAnios[i][idxMonth]
				days = limitDays[1][idxMonth] - limitDays[0][idxMonth] + 1
				if pmax < (0.85 * x0a[i]):
					powerCost[i] += (0.85 * x0a[i]) * powerDayCost[i] * days
				elif (pmax >= (0.85 * x0a[i])) and pmax < (1.05 * x0a[i]):
					powerCost[i] += pmax * powerDayCost[i] * days
				else:
					powerCost[i] += (2*(pmax - 1.05 * x0a[i]) + pmax) * powerDayCost[i] * days
	fx = sum(powerCost)	
	return fx
...
def optimize_cons(conexion, x0):
	...
	if(conexion['rate'] == 3):
		cons = ({'type': 'ineq',
			...
			{'type': 'ineq',
				'fun' : lambda x: np.array([450 - x[2]]),
				'jac' : lambda x: np.array([0.0, 0.0, -1.0])})
	else:
		cons = ({'type': 'ineq',
				'fun' : lambda x: np.array([x[0] - 16]),
				'jac' : lambda x: np.array([1.0, 1.0, 0.0])},
			{'type': 'ineq',
				'fun' : lambda x: np.array([x[1] - 16]),
				'jac' : lambda x: np.array([0.0, 1.0, 1.0])},		
			{'type': 'ineq',
				'fun' : lambda x: np.array([x[2] - 16]),
				'jac' : lambda x: np.array([0.0, 0.0, 1.0])})

	bnds = [[1.0, 10000000], [1.0, 10000000], [1.0, 10000000]]
	try:
...
def main(argv):
	...
	try:
	  opts, args = getopt.getopt(argv,"hs:p:u:k:d:b:e:o:",["server=","port=","username=","password=","database=","begin_date=", "end_date=", "rate_option="])
	except getopt.GetoptError:
	  print 'optimization_3.py -s <server_address> -p <port> -u <username> -k <password> -d <database> -b <begin_date> -e <end_date> -o <rate_option>'
	  sys.exit(2)
	for opt, arg in opts:
		if opt == '-h':
			print 'optimization_3.py -s <server> -p<port> -u <username> -k <password> -d <database> -b <begin_date> -e <end_date> -o <rate_option>'
		 	sys.exit()
			...
			dates['end_date'] = arg
		elif opt in ("-o", "--rate_option"):
			conexion['rate'] = int(arg)
