...
class SendInformesAnalizadorMensual extends Command
{
  /**
   ...
   * @var string
   */
  protected $signature = 'send:InformesAnalizadorMensual';
   ...
   * @var string
   */
  protected $description = 'Programacion Mensual';
  ...
  {
    $minut = Informes_analizadores::where('check', 3)->get();
    if (!is_null($minut)) {
      $informes = "Mensual";
      ...
      foreach ($minut as $key) {
        ...
        foreach ($analyzer as $i => $value) {
          ...
          $date_mes = date("m", strtotime($date_from));
          $label_intervalo = $months_T[(int)$date_mes] . " " . date("Y", strtotime($date_from)) . "";
          ...
          for ($j = 0; $j < 13; $j++) {
            $date_from = \Carbon\Carbon::now()->subMonths(1 + $j)->startOfMonth()->toDateString();
            $date_to = \Carbon\Carbon::now()->subMonths(1 + $j)->endOfMonth()->toDateString();
            $tb_data = $db->table('Analizadores_Tipo')->select(\DB::raw("`ENEact (kWh)` energia_activa,
                                            `ENErea (kVArh)` AS energia_reactiva, `POWact_Total (kW)` POWact_Total, `POWrea_Total (kVAr)` POWrea_Total, `IAC1 (A)` IAC1, `IAC2 (A)` IAC2, `IAC3 (A)` IAC3,`PF_Total` PF_Total "))
              ->where('date', '>=', $date_from)->where('date', '<=', $date_to)->orderBy("date", "ASC")
              ->orderBy("time", "ASC")->get();
            ...
            $date_mes = date("m", strtotime($date_from));
            $data_analizadores_total[$j]['date'] = $months_T[(int)$date_mes] . " " . date("Y", strtotime($date_from)) . "";
