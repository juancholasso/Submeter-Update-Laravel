...
class SendInformesTrimestre extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'send:trismestral';
     ...
     * @var string
     */
    protected $description = 'Envio de informes intervalo trimestral';
    ...
    public function handle()
    {
        ...
        $minut = Informes::where('check',4)->get();
        if(!is_null($minut))
        {
                ...
                foreach ($mails as $value)
                {
                    ...
                    if($tipo_count < 3)
                    {
                        if($tipo_tarifa == 1)
                        {
                            ...
                            // dd($total_ventas,$db_Venta_Energia);
                    ...
                    $label_intervalo = ceil(date("m", strtotime($date_from))/3)." Trimestre ".date("Y", strtotime($date_from))."";
    ...
    public function getHistoricalComparisonConsumption($db)
    {
        ...
        if($now == 1 || $now == 2 || $now == 3)
        {
            $date_from = Carbon::now()->subMonths($now+2)->startOfMonth()->toDateString();
            $date_to = Carbon::now()->subMonths($now)->endOfMonth()->toDateString();
        }
        elseif($now == 4 || $now == 7 || $now == 10)
        {
            $date_from = Carbon::now()->subMonths(3)->startOfMonth()->toDateString();
            $date_to = Carbon::now()->subMonths(1)->endOfMonth()->toDateString();
        }
        elseif($now == 5 || $now == 8 || $now == 11)
        {
            $date_from = Carbon::now()->subMonths(4)->startOfMonth()->toDateString();
            $date_to = Carbon::now()->subMonths(2)->endOfMonth()->toDateString();
        }
        elseif($now == 6 || $now == 9 || $now == 12)
        {
            $date_from = Carbon::now()->subMonths(5)->startOfMonth()->toDateString();
            $date_to = Carbon::now()->subMonths(3)->endOfMonth()->toDateString();
        }
        ...
        $dt2 = Carbon::instance(new \DateTime("{$date_from} 00:00:00", new \DateTimeZone(config('app.timezone'))));
        
        for($i = 1 ; $i <= $maxCycles ; $i++){
            // Restamos un dia para ubicarnos en el trimestre anterior
            $date_to = $dt2->subDay()->toDateString();
            $date_from = $dt2->subMonths(2)->startOfMonth()->toDateString();
        ...
        $dt1 = Carbon::instance(new \DateTime("{$start}-01 00:00:00", new \DateTimeZone(config('app.timezone'))));
        $dt2 = Carbon::instance(new \DateTime("{$end}-01 00:00:00", new \DateTimeZone(config('app.timezone'))));
        ...
        foreach($cycles as $cycle){
            ...
            $dtto = str_replace("-", "", $cycle['to']);
            $dtfrom = str_replace("-", "", $cycle['from']);
            $k = "{$cycle['from']}@{$cycle['to']}";

            foreach($consumosActiva as $consumo){

                $dt6 = str_replace("-", "", $consumo->date);

                if((int)$dt6 >= (int)$dtfrom && $dt6 <= (int)$dtto){
    ...
    public function dateFormat($str){
        ...
        if(array_key_exists($p1[1], $months_T) && array_key_exists($p2[1], $months_T)){
            $_s1 = $months_T[$p1[1]];
            $_s2 = $months_T[$p2[1]];

            return "{$_s1} {$p1[0]} - {$_s2} {$p2[0]}";
        }
