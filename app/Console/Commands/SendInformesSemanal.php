...
class SendInformesSemanal extends Command
{
    /**
    ...
    public function getHistoricalComparisonConsumption($db)
    {
        ...
        for($i = 1 ; $i <= $maxCycles+1 ; $i++){
            ...
            $cycles[] = $dt1->subWeek()->format('Y-m-d');
        }
        ...
        $dt2 = Carbon::instance(new \DateTime("{$end} 00:00:00", new \DateTimeZone(config('app.timezone'))));

        $date_from = $dt1->startOfWeek()->format('Y-m-d');
        $date_to = $dt2->endOfWeek()->format('Y-m-d');
        ...
        foreach($cycles as $cycle){
            ...
            $w1 = Carbon::instance(new \DateTime("{$cycle} 00:00:00", new \DateTimeZone(config('app.timezone'))));

            foreach($consumosActiva as $consumo){
                $w2 = Carbon::instance(new \DateTime("{$consumo->date} 00:00:00", new \DateTimeZone(config('app.timezone'))));

                if($w1->weekOfYear === $w2->weekOfYear){
                    $acum1[$w1->weekOfYear][] = [
                        'E_Date'        => $consumo->date,
        ...
        foreach($cycles as $cycle){
            ...
            foreach($potencias as $potencia){
                $w2 = Carbon::instance(new \DateTime("{$potencia->date} 00:00:00", new \DateTimeZone(config('app.timezone'))));
