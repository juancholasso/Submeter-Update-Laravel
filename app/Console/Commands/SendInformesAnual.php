...
class SendInformesAnual extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'send:anual';
    ...
    public function handle()
    {
        ...
        $minut = Informes::where('check',5)->get();
        if(!is_null($minut))
        {
    ...
    public function getHistoricalComparisonConsumption($db)
    {
        ...
        for($i = 1 ; $i <= $maxCycles+1 ; $i++){
            ...
            $cycles[] = $dt1->subYear()->format('Y');
        }

        $start = $cycles[$maxCycles];
        $end = $cycles[0];

        $dt1 = Carbon::instance(new \DateTime("{$start}-01-01 00:00:00", new \DateTimeZone(config('app.timezone'))));
        $dt2 = Carbon::instance(new \DateTime("{$end}-12-31 00:00:00", new \DateTimeZone(config('app.timezone'))));
