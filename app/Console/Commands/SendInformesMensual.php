...
class SendInformesMensual extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'send:informesMemensual';
     ...
     * @var string
     */
    protected $description = 'Envío de Informes mensuales';
    ...
    public function handle()
    {
        ...
        $minut = Informes::where('check',3)->get();
        ...
        if(!is_null($minut))
        {
            foreach ($minut as $key) {
                ...
                foreach ($mails as $value)
                {
                    ...
                    $label_intervalo = date("m", strtotime($date_from));
                    $label_intervalo = $months_T[(int)$label_intervalo]." ".date("Y", strtotime($date_from))."";
    ...
    public function getHistoricalComparisonConsumption($db)
    {
        ...
        for($i = 1 ; $i <= $maxCycles+1 ; $i++){
            ...
            $cycles[] = $dt1->subMonth()->format('Y-m');
        }
        ...
        $date_to = $dt2->endOfMonth()->format('Y-m-d');
    ...
    public function dateFormat($str){
        ...
        if(array_key_exists($p[1], $months_T)){
            $_s = $months_T[$p[1]];
