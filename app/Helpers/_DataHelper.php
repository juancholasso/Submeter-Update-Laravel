...
class DataHelper
{
    ...
    public static function format_data($config)
    {
        ...
        foreach ($axes as $axe_key=>$axe) 
        {
            ...
            {
                $val = DataHelper::get_value($f_row,$field);
    ...
    private static function get_value($f_row,$field)
    {
        $field_values = [];
    ...
    public static function runUpsert($handle,$database,$table,$field,$key,$value)
    {
        ...
        $tupdate = "UPDATE `".$database."`.`".$table."` SET `".$field."`=? WHERE   date(`date`)= date('".$key."')";
        $tinsert = "INSERT INTO `".$database."`.`".$table."` (`date`, `".$field."`) VALUES (?, ?)";
        $tselect = "select 1 from  `".$database."`.`".$table."` WHERE   date(`date`)= date('".$key."')";
